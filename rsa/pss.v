...
pub fn verify_pss(pubkey PublicKey, mut h hash.Hash, digest []u8, sig []u8, opts PSSOptions) ! {
	...
	mut em := encrypt(pubkey, sig) or { return ErrVerification{} }
...
// sign_pss_with_salt calculates the signature of hashed using PSS with specified salt.
// Note that hashed must be the result of hashing the input message using the
// given hash function. salt is a random sequence of bytes whose length will be
// later used to verify the signature.
pub fn sign_pss_with_salt(priv PrivateKey, mut h hash.Hash, hashed []u8, salt []u8) ![]u8 {
